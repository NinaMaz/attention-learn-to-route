    ...
    def eval_model_bat(bat):
        with torch.no_grad():
            ...
            for n,i in enumerate(subroutes):
                temp = []
                for j in i:
                    temp.append(bat_with_depot[n].gather(0, j.unsqueeze(-1).expand_as(bat_with_depot[n])))
                l.append(temp)
            cost = train_batch(
                tsp_model, l
            )
            cost = torch.stack(cost, dim = 0)#torch.tensor(cost).unsqueeze(-1).to(opts.device)
...
def train_epoch(
    ...
    for batch_id, batch in enumerate(
        ...
        subroutes, bl_val, x, log_likelihood = division_model_forward(
            division_model, optimizer, baseline, epoch, batch_id, step, batch, tb_logger, opts
...
def train_batch(
    ...
    results = []
    new_subroutes = [torch.stack(b, dim=0) for b in subroutes]
    for subproblem in new_subroutes:
        #[batch,n,21,2]
            
        with torch.no_grad():
            batch_rep = 1
            iter_rep = 1
            costs, log_likelihood = model(subproblem)
            batch_size = len(costs)
            ids = torch.arange(batch_size, dtype=torch.int64, device=costs.device)


            
            results.append(torch.sum(costs, dim = 0))
    return(results)
...
def division_model_forward(
    ...
    for n,i in enumerate(subroutes):
        temp = []
        for j in i:
            temp.append(x_with_depot[n].gather(0, j.unsqueeze(-1).expand_as(x_with_depot[n])))
        l.append(temp)
